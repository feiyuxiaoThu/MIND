...
基于 configs/demo_3.json 的场景演示。
目标速度: 8 m/s，enable_timestep: 0.0，适合高速公路场景。
"""
...
def create_demo_3_config():
    ...
    config = {
        ...
        'constraints': {
            'state_upper_bound': np.array([100000.0, 100000.0, 15.0, 10.0, 3.0, 0.15]),
            'state_lower_bound': np.array([-100000.0, -100000.0, 0.0, -10.0, -4.0, -0.15])
        },
        'weights': {
...
def create_highway_scenario():
    """创建高速公路场景"""
    print("创建Demo 3 - 高速公路场景")
    print("目标速度: 8 m/s，序列ID: 08a8b0c9-f93f-4ade-bcaa-e5348aeca381")
    print("启用时间步: 0.0 (立即开始)")
    
    config = create_demo_3_config()
    planner = MINDPlanner(config)
    
    # 自车状态 - 高速公路行驶
    ego_state = np.array([0.0, 0.0, 8.0, 0.0, 0.0, 0.0])
    ...
    for t in range(horizon):
        # 高速公路保持稳定速度
        speed = 8.0 + 0.5 * np.sin(t * 0.05)  # 轻微速度波动
    ...
    for t in range(horizon):
        if t < 30:  # 正常行驶
            ...
            speed = max(2.0, 8.0 - 0.3 * (t - 30))
            means3[t] = [speed * t * 0.2, 0.0]
        covs3[t] = 0.1 * np.eye(2)
    predictions.append(AgentPrediction(means3, covs3, 0.30))
    
    # 目标车道 - 高速公路直行车道
    target_lane = np.array([[x, 0.0] for x in np.linspace(0, 120, 150)])
...
def visualize_demo_3_scenario():
    """可视化Demo 3场景"""
    planner, ego_state, predictions, target_lane, road_data = create_highway_scenario()
    
    fig, axes = plt.subplots(2, 2, figsize=(15, 12))
    fig.suptitle('Demo 3 - 高速公路场景 (目标速度: 8 m/s)', fontsize=16, fontweight='bold')
    ...
    # 绘制道路特征
    # 上匝道
    on_ramp = road_data['features']['on_ramp']
    ax.arrow(on_ramp['position'][0]-10, on_ramp['position'][1], 
            10, 3.5, head_width=1, head_length=2, fc='gray', ec='gray')
    ax.text(on_ramp['position'][0]-5, on_ramp['position'][1]-2, "上匝道", fontsize=10)
    
    # 下匝道
    off_ramp = road_data['features']['off_ramp']
    ax.arrow(off_ramp['position'][0], off_ramp['position'][1], 
            10, 3.5, head_width=1, head_length=2, fc='gray', ec='gray')
    ax.text(off_ramp['position'][0]+5, off_ramp['position'][1]+2, "下匝道", fontsize=10)
    
    # 隧道
    tunnel = road_data['features']['tunnel']
    ax.add_patch(plt.Rectangle((tunnel['start'], -5), tunnel['end']-tunnel['start'], 10,
                             facecolor='lightgray', alpha=0.3))
    ax.text((tunnel['start']+tunnel['end'])/2, -6, "隧道", fontsize=10, ha='center')
    ...
    if success and best_traj:
        ...
        ax.axhline(y=12.0, color='g', linestyle=':', label='限速')
        ax.set_xlabel('时间 (s)')
...
目标速度: 8.0 m/s
场景类型: 高速公路
启用时间步: {road_data['enable_timestep']}
...
- 隧道长度: {road_data['features']['tunnel']['end'] - road_data['features']['tunnel']['start']} m
"""
    else:
        text = "规划失败，无统计信息"
    ...
    filename = os.path.join(plot_dir, 'demo_3_scenario.png')
    plt.savefig(filename, dpi=150, bbox_inches='tight')
    print(f"Demo 3 场景可视化已保存到 {filename}")
...
def main():
    """主函数"""
    print("MIND Algorithm - Demo 3 场景演示")
    print("=" * 60)
    print("配置: configs/demo_3.json")
    print("序列ID: 08a8b0c9-f93f-4ade-bcaa-e5348aeca381")
    print("目标速度: 8 m/s")
    print("启用时间步: 0.0")
    print("=" * 60)
    
    # 创建并运行场景
    result = visualize_demo_3_scenario()
